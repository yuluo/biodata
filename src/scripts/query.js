...
//
// The dataset is exposed as a view named `variants`.
// Usage:
//   npm run query -- "SELECT COUNT(*) FROM variants"
//   npm run query -- "SELECT CHR,POS,GENE FROM variants WHERE GENE='BRCA1' LIMIT 5"
//
...
//   --parquet PATH override parquet glob (default: data_parquet/**/*.parquet)

const path = require('path');
const duckdb = require('duckdb');

const ROOT = path.resolve(__dirname, '..', '..');
const DEFAULT_GLOB = path.join(ROOT, 'data_parquet', '**', '*.parquet');
...
conn.exec(`PRAGMA threads=${threads};`, (err) => {
  ...
  conn.exec(
    `CREATE VIEW variants AS
     SELECT * FROM read_parquet('${parquetGlob.replace(/'/g, "''")}', hive_partitioning=true);`,
    (err2) => {
      if (err2) {
